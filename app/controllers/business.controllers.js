...
async function checkDomain(req, res, next) {
    try {
        ...
        return res.send({
            success: true,
            message: "The domain and keys are correct.",
        });
    } catch (e) {
        next(e);
    }
...
async function create(req, res, next) {
    try {
        ...
        const { success: isCategoriesLoaded, categories } =
            await wc.getAllCategories();
        ...
        const {
            success: isProductLoaded,
            products,
            variations,
        } = await wc.getAllProducts();
        if (!isProductLoaded) {
            throw new BaseErr(
        ...
        const { success: isCustomersLoaded, customers } =
            await wc.getAllCustomers();
        if (!isCustomersLoaded) {
            throw new BaseErr(
                "WoocommerceGetCustomersFailed",
                httpStatusCodes.NOT_ACCEPTABLE,
                true,
                `Cannot fetch customers.`
            );
        }
        ...
        for (const category of categories) {
            if (!!category.parent) {
                const newParent = await Category.findOne({
                    where: { ref: category.parent },
                });
                if (newParent) {
                    const isCategoryExisted = await Category.findOne({
                        where: { ref: category.id },
                    });
                    if (!isCategoryExisted) {
                        await insertCategoryInDB(
                            category,
                            business.id,
                            newParent.id
                        );
                    ...
                    const parentCreated = await insertCategoryInDB(
                        parent,
                        business.id
                    );
                    await insertCategoryInDB(
        ...
        for (const order of orders) {
            ...
            const createdOrder = await Order.create({
                ...
                discountTotal: order.discount_total
                    ? Math.floor((order.discount_total * 100) / order.total)
                    : 0,
        ...
        return res.json({
            success: true,
            message:
                "The categories, tags, products and orders have loaded successfully.",
        });
    } catch (e) {
        next(e);
    }
