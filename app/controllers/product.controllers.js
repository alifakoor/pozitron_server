...
async function create(req, res, next) {
    try {
        ...
        req.body["salePrice"] = calculateSalePrice(
            req.body.price,
            req.body.discount
        );
        req.body["onlineSalePrice"] = calculateSalePrice(
            req.body.onlinePrice,
            req.body.onlineDiscount
        );
        req.body["dimension"] = calculateDimension(
            req.body.length,
            req.body.width,
            req.body.height
        );
        ...
        if (req.body.type === "variable") {
            for (const variation of req.body.variations) {
                variation["salePrice"] = calculateSalePrice(
                    variation.price,
                    variation.discount
                );
                variation["onlineSalePrice"] = calculateSalePrice(
                    variation.onlinePrice,
                    variation.onlineDiscount
                );
...
async function removeUpload(req, res, next) {
    try {
        ...
        await fs.unlink(
            path.join(
                __dirname,
                `../public/uploads/products/${req.params.name}`
            )
        );
...
async function logWebhookResponse(webhook, body) {
    try {
        ...
        await fs.appendFile(
            path.join(__dirname, `../logs/webhooks/product/${+date}.json`),
            JSON.stringify(data),
            "utf-8"
        );
    } catch (err) {
...
async function createdWithWebhook(req, res, next) {
    try {
        ...
        const product = {
            ...
            onlineDiscount: calculateDiscount(
        ...
        if (req.body.type === "variation") {
            const parent = await Product.findOne({
                where: { ref: req.body.parent_id, businessId },
            });
...
async function updatedWithWebhook(req, res, next) {
    try {
        ...
        return res.json({
            success: false,
            message: "The products has been updated successfully. ",
        });
    } catch (e) {
        next(e);
...
async function deletedWithWebhook(req, res, next) {
    try {
        ...
        const product = await Product.findOne({
            where: { ref: req.body.id, businessId },
        });
        if (!product) {
            return res.send({
                success: false,
